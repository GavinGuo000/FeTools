<template>
    <div class="mock-wrap">
        <nav class="mock-top">
            ...
            <a-button @click="exportData" class="btn-wrap">
                <a-icon type="download" /> 导出配置
            </a-button>
            <a-button @click="triggerImport" class="btn-wrap">
                <a-icon type="upload" /> 导入配置
            </a-button>
            <input ref="importInput" type="file" accept="application/json" class="import-input" @change="importData" />
        </nav>
        <section>
            <a-table :columns="columns" :data-source="listData" rowKey="key">
        ...
        <a-modal v-model="visible" title="mock数据详情" okText="确定" cancelText="取消" @ok="handleOk" @cancel="handleCancel">
            ...
            <div class="mock-wrap-inline">
                <label>响应延迟(ms)：</label>
                <a-input placeholder="如 1000，模拟慢请求，留空则不延迟" v-model="delay" />
            </div>
            <div class="mock-wrap-inline">
                <label>自定义数据：</label>
                <vue-json-editor
    ...
    methods: {
        ...
        exportData() {
            ...
            chrome.storage.local.get(['listData'], result => {
                const list = result.listData || [];
                if (!list.length) {
                    me.$message.warning('暂无可导出的mock数据');
                    return;
                }
                const blob = new Blob([JSON.stringify(list, null, 2)], {type: 'application/json'});
                const a = document.createElement('a');
                a.href = URL.createObjectURL(blob);
                a.download = `fetools-mock-${Date.now()}.json`;
                a.click();
        ...
        importData(e) {
            ...
            reader.onload = ev => {
                try {
                    ...
                    chrome.storage.local.get(['listData'], result => {
                        const list = (result.listData || []).concat(imported.map(item => ({
                            ...item,
                            key: Math.random() * 10e16
                        })));
                        chrome.storage.local.set({listData: list}, () => {
                            me.listData = list;
                            me.$message.success(`成功导入 ${imported.length} 条mock数据！`);
                        });
                    });
                }
                catch (error) {
                    me.$message.error('导入失败：文件不是合法的mock配置');
                }
            };
